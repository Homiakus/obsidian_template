...
export class Analytics {
  ...
  getPerformanceAnalytics(): Record<string, any> {
    ...
    if (perfEvents.length > 0) {
      const durations = perfEvents.map(e => e.data?.duration || 0);
      perfStats.averageDuration = durations.reduce((a, b) => a + b, 0) / durations.length;
      ...
      perfEvents.forEach(event => {
        ...
        if (operation) {
          ...
          perfStats.operationsByType[operation].count++;
          perfStats.operationsByType[operation].totalDuration += duration;
        }
      });
      ...
      const sortedByDuration = perfEvents
        .map(e => ({ operation: e.data?.operation, duration: e.data?.duration || 0 }))
